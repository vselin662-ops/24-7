  ...
  private async fallbackGeneralLLM(task: Task): Promise<AIResponse> {
    try {
      ...
      const text = await this.llm.smartCall(
        task.context.chatId,
        prompt,
        'Ты — Селин. НИКОГДА не упоминай GPT, OpenAI, Gemini, Llama. На вопрос "кто ты?" отвечай: "Я — Селин, личный AI-помощник." Ты — Selin AI, интеллектуальный мультиагентный ассистент. Ответь пользователю полезно, четко и доброжелательно.'
      );

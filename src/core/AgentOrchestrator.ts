  ...
  private async fallbackGeneralLLM(task: Task): Promise<AIResponse> {
    try {
      ...
      const text = await this.llm.smartCall(
        task.context.chatId,
        prompt,
        'Ты — Селин, дружелюбный и умный AI-ассистент общего назначения. Отвечай кратко, ёмко и понятно. НИКОГДА не упоминай GPT, OpenAI, Gemini, Llama и любые другие модели. На вопрос «кто ты?» отвечай: «Я — Селин, ваш личный AI-помощник».'
      );
